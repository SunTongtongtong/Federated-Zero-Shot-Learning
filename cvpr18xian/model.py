...
class MLP_G(nn.Module):
    def __init__(self, opt):
        ...
        self.fc1 = nn.Linear(opt.attSize+ 512 + opt.nz, opt.ngh)
        self.fc2 = nn.Linear(opt.ngh, opt.resSize)
        self.lrelu = nn.LeakyReLU(0.2, True)
        #self.prelu = nn.PReLU()
        self.relu = nn.ReLU(True)
...
class MLP_compressCLIP(nn.Module):
    def __init__(self,opt):
        super(MLP_compressCLIP,self).__init__()
        if opt.clip_noise:
            self.fc1 = nn.Linear(1024,512)
            self.fc2 = nn.Linear(512,opt.attSize)

        else:
            self.fc1 = nn.Linear(opt.attSize+512,opt.attSize)
            # self.fc2 = nn.Linear(256,85)
        self.lrelu = nn.LeakyReLU(0.2,True)
        self.apply(weights_init)
